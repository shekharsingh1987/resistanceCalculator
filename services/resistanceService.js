...
var GetColorCodeDigitForBand = function(input, band) {
  ...
  for (var ca = 0; ca < colorArray.length; ca++) {
    if (colorArray[ca]) {
      var colorValue = colorCodeDataHelper.GetColorStandardDigitValue(
        colorArray[ca]
      );
      if (colorValue) {
        if (ca < band - 2) {
  ...
  if (input) {
    ...
    if (responseResistance.length > 1) {
      ...
      if (significantLength == -1) {
        ...
        var colorTableObject = colorCodeDataHelper.GetColorBandNameByValue(
          element
        );
        ...
        if(colorTableObjectForSignificant){
    ...
    botService.process(
      ...
      function(aiResult) {
        if (aiResult.fulfillmentText) {
          var band = aiResult.intent.displayName.split("_");
          if (band.length > 1) {
            getResistance(band, aiResult.fulfillmentText, function(
              jsonResponse
            ) {
              res.json(jsonResponse);
            });
          } else if (band[0] == "ResistorColorCode") {
            getResistance(band, aiResult.fulfillmentText, function(
              jsonResponse
            ) {
              res.json(jsonResponse);
            ...
            res.json({
              success: false,
              content:
                "We couldn't understand intent,Please try again saying in correct format."
            });
          }
        } else {
          res.json({
            success: false,
            content: "Please try again saying in correct format."
          });
        }
      },
      function(error) {
